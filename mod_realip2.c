/*
 * * mod_realip2 * Olexander Shtepa * Changelog: *  1.1 - Move the
 * `realip2_post_read_request' handler from being APR_HOOK_MIDDLE to
 * APR_HOOK_FIRST to make the module run before modules like mod_geoip. (from
 * mod_rpaf) *  1.0 - Initial release.
 */
...
static int 
realip2_match_proxy(const request_rec * r, const realip2_server_cfg *
		    cfg)
{
	int		i;
...
static void 
realip2_replace_ip(request_rec * r, const char *new_ip)
{
...
static int 
realip2_post_read_request(request_rec * r)
{
...
#ifdef REALIP2_VERSION_SHOW
static int 
realip2_post_config(apr_pool_t * pconf, apr_pool_t * plog, apr_pool_t *
		    ptemp, server_rec * s)
{
...
static void    *
realip2_create_server_config(apr_pool_t * p, server_rec * s)
{
	realip2_server_cfg *
...
static void    *
realip2_merge_server_config(apr_pool_t * p, void *def_cfg, void *
			    new_cfg)
{
...
static const char *
realip2_set_realip(cmd_parms * cmd, void *mconfig, int value)
{
	realip2_server_cfg *
...
static const char *
realip2_set_realipproxy(cmd_parms * cmd, void *mconfig, const
			char *host)
{
	apr_status_t	err;
	realip2_server_cfg *
...
static const char *
realip2_set_realipheader(cmd_parms * cmd, void *mconfig,
			 const char *header)
{
	realip2_server_cfg *
			cfg = (realip2_server_cfg *) ap_get_module_config(cmd->server->module_config, &realip2_module);
...
static void 
realip2_register_hooks(apr_pool_t * p)
{
#ifdef REALIP2_VERSION_SHOW
